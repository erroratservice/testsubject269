    ...
    try:
        ...
        async def remove_trackers_like_source():
            try:
                ...
                # Get current trackers and operate on a single pass like the source
                ...
                tail = trackers[3:] if len(trackers) > 3 else []

                # Find index of first failing tracker (status 2 or 3)
                start_idx = None
                for i, t in enumerate(tail):
                    if getattr(t, "status", None) in (2, 3):
                        start_idx = i
                        break

                # If we found a failing tracker, remove it and everything after it
                if start_idx is not None:
                    urls_to_remove = [t.url for t in tail[start_idx:] if getattr(t, "url", None)]
                    if urls_to_remove:
                        try:
                            await sync_to_async(
                            ...
                            # Fallback for older client param handling
                            await sync_to_async(
                                qbittorrent_client.torrents_remove_trackers,
                                torrent_hash=ext_hash,
                                urls="\n".join(urls_to_remove),
                            )
                        rm_urls.extend(urls_to_remove)
