    ...
    if task := await get_task_by_gid(tor.hash[:12]):
        if task.listener.stop_duplicate:
            task.listener.name = tor.content_path.rsplit("/", 1)[-1].rsplit(".!qB", 1)[
                0
            ] if hasattr(tor, "content_path") else task.listener.name
...
async def _on_download_complete(tor):
    ...
    if task := await get_task_by_gid(ext_hash[:12]):
        ...
        if task.listener.select:
            # Clean unwanted outside the torrent tree if used internally elsewhere
            await clean_unwanted(task.listener.dir)
            # Build base directory portably for 4.2.5 (no content_path in API)
            # Use save_path + name for multi-file, else save_path for single-file
            files = await sync_to_async(
                qbittorrent_client.torrents_files, torrent_hash=ext_hash
            )
            try:
                # Detect multi-file by presence of '/' in any relative file name
                is_multifile = any("/" in f.name for f in files)
            except Exception:
                is_multifile = False
            try:
                base_dir = os.path.join(tor.save_path, tor.name) if is_multifile else tor.save_path
            except Exception:
                base_dir = tor.save_path
            # Remove all unselected (priority 0) files by relative paths
            for f in files:
                try:
                    if f.priority == 0:
                        target = f"{base_dir}/{f.name}"
        ...
        async with qb_listener_lock:
            try:
                ...
                for tor_info in torrents:
                    ...
                    # Enhanced completion detection for 4.2.5
                    elif (
